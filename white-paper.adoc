...
Ident. Die GLN dient vier Funktionen, sie ist als Party GLN eine Identifikationsnummer für Unternehmen, insbesondere für
diejenigen, die an GS1 Complete teilnehmen (juristische Entität). Es ist
möglich mit der GLN verschiedene Unternehmensbereiche zu
identifizieren, was mit einer LEI verboten ist. Somit dient sie entsprechend auch der Identifikation
von physischen sowie digitalen Lokationen. Weiterhin kann man auch Funktionen, das bedeutet einzelne Abteilungen oder Aufgaben, 
identifizieren.

Die GS1 Company Prefix (GCP)
wird von GS1 Germany selbstständig
...
in der Basisnummer von sieben, acht oder neun Stellen
...
[[sec_pki]]
=== Zentrale Vertrauensinfrastruktur
...
Aufbau einer "Public Key Infrastructure" (PKI). Durch das enorme
Wachstum und die Verbreitung des World Wide Web stieg auch der Bedarf
an Authentifizierung der beteiligten Menschen und Server, welche für
eine sicherer Kommunikation unabdingbar ist, stark an. Alleine aus
kommerziellen Gründen wie z.B. E-Commerce, Banking, Social Networks
oder Online-Zugriff auf Datenbanken wird eine PKI unabdingbar.

Die klassische zentrale PKI besteht aus 3 Funktionen: Eine Registrierungsstelle,
...
==== Certified by GS1 Trust Model Das Vertrauen in Idente certified by GS1
funktioniert konzeptionell ähnlich wie bei klassischen Dokumenten. Auch wenn die
entsprechenden Ideen aus dem DID/SSI Umfeld kommen, ist das Trust-Modell
unabhängig von der Realisierung über DIDs oder klassische (zentrale) PKI
Zertifikatsketten. Ausweisketten
...
Dokument, etc.) eines fälschungssicheren Ausweises ("Autorisierung") zustande.
...
und zusätzlich, dass der Inhaber derjenige ist, für den das Dokument ausgestellt
wurde. So wird das Vertrauen des Prüfers, wiederum indirekt, auf den Inhaber des
Dokuments übertragen und die Autorisierung ist erfolgreich. Der Prozess wird in
Bild 1  dargestellt.
...
Ausweisdokumente, wie in Bild 2 analog zu Bild 1 dargestellt. Wichtig ist
hierbei, dass lediglich zwischen GS1 und dem Maschinenhersteller ein
Vertragsverhältnis existieren muss. Es bedarf prinzipiell keiner direkten
Verbindung zwischen dem Fabrikbetreiber und GS1 oder [oder????] -> geht beides,
Stichwort GIAI
