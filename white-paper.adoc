...
In der physischen Welt wird dies bereits seit Anfang des 19. Jahrhunderts durch Identitätssysteme wie z.B. den Personalausweis unterstützt. Im Zuge der Digitalisierung werden jedoch immer mehr soziale und wirtschaftliche Interaktionen von der physischen in die digitale Welt verlagert. In dieser Welt helfen physische Identitätssysteme nicht in gleichem Maße, aber die Fähigkeit, eine vertrauensvolle Beziehung zwischen den interagierenden Parteien im digitalen Raum aufzubauen, ist ebenso entscheidend, um Geschäfte zu ermöglichen. 
...
Das Internet wurde ohne eine standardisierte, explizite Möglichkeit zur Identifizierung von Personen oder Organisationen aufgebaut wurde und somit entstand eine Vielzahl von Lösungen, um den Anforderungen des Identitätsmanagements gerecht zu werden.
...
Die Realisierung auf Basis von SSI Technologien bietet den großen
...
DID::
Ein Decentralized Identifier (DID) ist eine Art Ident, der eine überprüfbare, dezentralisierte digitale Identität ermöglicht.
...
Der Electronic Product Code (EPC) ist ein Identifikationsschema für die universelle Identifikation physischer Objekte mittels RFID-Tags.
...
GEPIR::
Die Global Electronic Party Information Registry (GEPIR) ist eine Schnittstelle für Web-Browser und Protokolle zur Verbindung zwischen den Datenbanken der nationalen GS1 Mitgliedsorganisationen.
...
Verified by GS1::
Stammdaten Qualität Service der GS1.
