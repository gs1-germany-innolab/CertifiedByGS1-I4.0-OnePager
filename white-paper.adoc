...
Im Rahmen der Digitalisierung im Allgemeinen und dem Internet of Things (IoT) im
Besonderen sind Sicherheitsprobleme, insbesondere im Bereich der sicheren
Identifikation, ein wesentliches Hindernis bei der Umsetzung interessanter
Anwendungsfälle. Gerade in den Industrie 4.0 Zukunfts-Szenarien, z.B. bei der
wandelbaren Fabrik, ist eine sichere Identifikation der Maschinen untereinander
eine essenzielle Voraussetzung für das korrekte Funktionieren und zum Schutz vor
Fälschungen. Um diese Herausforderungen zu lösen, wird unter anderem eine
vertrauenswürdige und sichere Identitätsinfrastruktur zur Authentifizierung von
Maschinen untereinander benötigt. Auch um Produktpiraterie und -fälschung zu
verhindern sind sichere Idente ein wichtiger Baustein.  
Im Rahmen des Projektes _Certified by GS1_ wurden Konzepte entwickelt, um das
bestehende System der GS1 Idente durch digitale Zertifikate abzusichern und so
automatisch authentifizierbar zu machen. Es wird aufgezeigt, wie die Stellung
der GS1 als vertrauenswürdige und neutrale not-for-profit Organisation zum
Identitätsanbieter und Vertrauensanker in einer digitalen
Vertrauensinfrastruktur ausgebaut werden kann, die im Rahmen verschiedener
Projekte auf Nationaler, vor allem aber internationaler Ebene aktuell entwickelt
und etabliert wird. 


In bestimmten Anwendungsfällen, insbesondere im Machine-To-Machine (M2M)
Bereich, ebenso aber auch in B2B und B2C Prozessen, wird eine weitgehende
Automatisierung von Abläufen dadurch erschwert, manchmal auch verhindert, dass
Kommunikationspartner nicht sicher automatisch authentifiziert werden können.
Durch eine Erweiterung der bestehenden GS1 Identifikationsschlüssel um
Sicherheits- und Authentifizierungsfunktionen, könnten _Certified by GS1_ Idente
im IoT-Umfeld die unternehmensübergreifende, eindeutige und sichere
Identifikation von Maschinen untereinander sicherstellen. Für diese wichtige
Voraussetzung eines _Internet_ of Things ist bisher kein alternatives System
etabliert. Über diese neuen Anwendungsfälle hinaus gibt es aber auch in gerade
den Bereichen, in denen GS1 Idente bereits heute weit verbreitet sind,
erhebliches Potential für zertifizierte Idente. Beim Stammdaten austausch zu
Wahren oder Unternehmen profitieren alle beteiligten von der zusätzlichen
Sicherheit die eine digitale Signatur bietet und im Kampf gegen Produktfälschung
können signierte und damit sicher authentische Idente einen wichtigen Beitrag
liefern.
...
[[Layers]]
.Vereinfachtes Schichten-Modell der SSI Technologie
image::CertifiedLayers.png[pdfwidth=95%,align="center"]



Ein wichtiger Baustein in jeder Vertrauensinfrastruktur ist die authentische und
sichere Verknüpfung von realen und digitalen Entitäten. Die Registrierung
digital zertifizierter Idente stellt neue Anforderungen an den Vergabeprozess.
Im Rahmen der Vergabe der GS1 Basisnummer (Global Company Prefix, GCP) wird die
Legitimität des Antrags, bzw. des Antragsstellers aktuell nicht aufwändig
überprüft. Dies ist auch nicht nötig und würde keinen Mehrwert bringen. Um eine
Vertrauensbasis für ein System *zertifizierter* GS1 Identen zu schaffen muss
jedoch ein Registrierungsprozess etabliert werden, der die Legitimität des
Antrags überprüft. Im Unterschied zum bestehenden Prozess wir bei der vergabe
einer Certified GCP nicht nur ein Nummernkreis einer Firma zugeordnet. Das zur
GCP gehörende digitale Zertifikat etabliert die Zugehörigkeit eines privaten
Schlüssels als Autorisierungsmerkmal zu einer Firma. Vertrauen in das System und
die Echtheit von zertifizierten Identen basiert darauf, dass diese initiale
Überprüfung der Identität der Firma sicher erfolgt. Dann, und nur dann, kann bei
nach erfolgreicher Überprüfung der digitalen Signatur, z.B. einer Certified
GTIN, der Prüfer sicher sein, dass der Identifikationsschlüssel in der Tat
legitim von der Firma erstellt wurde, welcher der Nummernkreis gehört. Die in
diesem Projekt untersuchte SSI Technologie stellt hier nicht nur die Technische
Möglichkeit zur Umsetzung der Certified Idente bereit. Ein hinreichend sicher
etabliertes SSI Netzwerk könnte diese Registrierungsfunktion für einen Certified
Antrag bereitstellen und so eine schnelle Umsetzung der hier vorgestellten Ideen
ermöglichen.



Auf Empfehlung der in- und externen Projektbeteiligten werden die im Projekt
erarbeiteten Konzepte und Demonstrationen, zu Prototypen und möglichst auch zu
einem Minimum Viable Product (MVP) weiterentwickelt. Sie können uns dabei
unterstützen und Teil dieser spannenden Entwicklung sein! Erfahren Sie alles
über das Konzept auf den nachfolgenden Seiten und
mailto:Certified@gs1.de[kontaktieren Sie uns gerne mit Fragen und Anregungen
...
Der Kernfokus von GS1 liegt in der Entwicklung von Standards, die die Interoperabilität und Digitalisierung zwischen Unternehmen, Produkten und Dingen, ermöglichen. Der Bedarf an standardisiertem sicheren Identitätsmanagement und sicheren Infrastrukturen, insbesondere mit der schnellen Verbreitung des IoTs, war folglich der Haupttreiber um das Projekt _Certified by GS1_ zu starten.
...
kann dann anderen Teilnehmern Zertifikate ausstellen (siehe <<sec_pki>> und <<sec_ssi>>).
...
certified by GS1 entwickelt und insbesondere in <<sec_ssi_proto>> ein Self Sovereign Identity Protokoll
...
=== Zertifikate zur Absicherung von Identen
...
Dieses System wird im Rahmen von _Certified by GS1_ auch für die 
...
können, einen großen Vorteil für den Hersteller dar. Dieser Ansatz
...
.Eve fängt die Kommunikation zwischen Alice und Bob ab. So kann Eve Alice seinen eigenen öffentlichen Schlüssel als den von Bob unterschieben. GLeichzeitig übergibt Eve Bob ebenfalls den eigenen Schlüssel im Namen von Alice. Ab diesem Zeitpunkt glauben Alice und Bob sicher und authentifiziert zu kommunizieren, aber in Wirklichkeit schlüsselt Eve alle Nachrichten um und kann diese nicht nur mitlesen sondern sogar verändern, ohne dass der Betrug auffällt.
...
[[pic_pki]]
.Die drei wesentlichen Funktionseinheiten einer PKI Infrastruktur: Die Registrierungsstelle prüft die Legitimität des Antrags, die Zertifizierungsstelle verwahrt den eigenen Schlüssel sicher und signiert damit die ausgestellten Zertifikate, der Verzeichnisdienst veröffentlicht Zertifikate und verwaltet die Rückruflisten. (Die Abbildung stammt aus einer Vorlesung der TU Darmstadt.)
...
vertrauenswürdig einstufen, wenn es eine Zertifikatskette 
von ihm selbst ausgehend gibt, die Susis Schlüssel als vertrauenswürdig
zertifiziert.
...
Controller kann aber auch z.B. ein entsprechend befugter Mitarbeiter sein, falls das Subjekt
...
Grundlage der Vertrauensprüfung ist die Prüfung des Antrags auf eine _certified_
GCP. Wenn aktuell jemand z.B. im Namen des EECC eine neue GCP beantragt (und
bezahlt), so wird dem EECC eine solche zugeteilt. Dadurch kann das EECC im
folgenden Identifikationsnummern, z.B. GTINs, in einem neuen Nummernkreis
vergeben. Ob eine GTIN im Nummernkreis des EECC tatsächlich legitim vom EECC
selbst vergeben wurde, oder lediglich syntaktisch korrekt von einem Dritten
erstellt wurde, lässt sich alleine anhand des Identifikationsschlüssels nicht
überprüfen. D.h. ist eine Überprüfung der Legitimität des Antrags auf eine GCP,
d.h. einen Nummernkreis, aktuell nicht notwendig. Ganz anders ist bei der
Vergabe einer _certified_ GCP die Überprüfung der legitimität des Antrags die
Basis für das Vertrauen in das hier vorgestellte Certified by GS1 System.
...
[[tab_VC_X]]
.Vergleich von X.509 Zertifikaten mit Verifiable Credentials
[cols="^.<,^.<,^.<",options="header"]
|===
|
...
- [[[glossar]]] GS1 Glossar https://www.gs1.at/glossar#S
...
- [[[id_key]]] https://www.computerlexikon.com/begriff-identifikationsschl%C3%BCssel
...
Barcode::
Ein Barcode ist ein Symbol, das Daten in ein maschinenlesbares Muster von parallel angrenzenden, in der Breite variierenden, dunklen Strichen und hellen Lücken verschlüsselt. Siehe <<glossar>>.

Certified by GS1::
Wird sowohl als Name des abgeschlossenen F&E Projekts als auch für die entwickelten Konzepte verwendet. 

DID::
Ein Decentralized Identifier (DID) ist ein weltweites Ident, der eine überprüfbare, dezentralisierte digitale Identität ermöglicht. Siehe <<did-core>>.

Digitale Signatur::
Ist ein mathematisches Schema zur Verifizierung der Authentizität von digitalen Nachrichten oder Dokumenten. Siehe <<sec_digi_sig>>.
...
EECC::
Der europäischen Marktführer für Lösungen und Services rund um den Electronic Product Code (EPC) und deren vernetzte Informationssysteme (EPCIS). Siehe <<eecc>>.

EPC (Electronic Product Code)::
Der Electronic Product Code (EPC) ist ein Identifikationsschema für die universelle Identifikation physischer Objekte mittels RF Tags. Siehe <<glossar>>.

EPC Tag oder RFID Tag::
Ein Tag, der auf den Standards von EPCglobal basiert und den Electronic Product Code enthält. Siehe <<glossar>>.
...
GEPIR::
Die Global Electronic Party Information Registry (GEPIR) ist eine Schnittstelle für Web-Browser und Protokolle zur Verbindung zwischen den Datenbanken der nationalen GS1 Mitgliedsorganisationen, um Unternehmensinformationen für einen gesuchten GS1 Identifikationsschlüssel oder auch für einzeln vergebene GS1 Identifikationsnummern zu liefern.  Siehe <<glossar>>.

GLN::
Die Global Location Number (GLN) wird zur Identifikation einer physischen Lokation oder Firma/­­Organisation verwendet. Siehe <<glossar>>.

GPC::
Der von GS1 gemeinsam mit Anwendern geschaffene Global Product Classification (GPC) Standard hilft Handelspartnern, Produkte überall auf der Welt auf die gleiche Weise zu gruppieren. Der Standard ist klar und international verständlich. Siehe <<glossar>>.

GS1::
GS1 hat seinen Hauptsitz in Brüssel, Belgien und Princeton, USA, und ist die Organisation, die das GS1 System verwaltet. GS1 ist die Dachorganisation, deren Mitglieder die nationalen GS1 Mitgliedsorganisationen sind. Siehe <<glossar>>.

GTIN (Global Trade Item Number)::
Die GTIN wird zur Identifikation einer Handelseinheit verwendet. Siehe <<glossar>>.

Identifikationsschlüssel::
Ein Schlüssel, dessen Wert jede Entität einer Entitätsmenge eindeutig identifiziert. Siehe <<id_key>>.

IoT (Internet of Things)::
ist ein Sammelbegriff für Technologien einer globalen Infrastruktur der Informationsgesellschaften, die es ermöglicht, physische und virtuelle Gegenstände miteinander zu vernetzen und sie durch Informations- und Kommunikationstechniken zusammenarbeiten zu lassen. Siehe <<iot>>.

PKI (Public Key Infrastructure)::
Mit PKI bezeichnet man in der Kryptologie ein System, das digitale Zertifikate ausstellen, verteilen und prüfen kann. Siehe <<pki>>.

SSI::
Self-sovereign identity (SSI) erlaubt es einer Person (oder einer Organisation) eine digitale Identität zu erzeugen und vollständig zu kontrollieren, ohne dass es der Erlaubnis eines Vermittlers oder einer zentralen Behörde bedarf. Siehe <<sec_sov>>.

Schlüssel::
Als Schlüssel wird in der Kryptologie eine Information bezeichnet, die einen kryptographischen Algorithmus parametrisiert und ihn so steuert. Siehe <<key>>.

SGTIN::
Serialisierte GTIN (Global Trade Item Number). Siehe <<glossar>>.

Stammdaten::
Stammdaten (engl. master data) sind Daten, die Grundinformationen über betrieblich relevante Objekte enthalten. Siehe <<glossar>>.

UUID (Universally Unique Identifier)::
ist eine 128-Bit-Zahl, welche zur Identifikation von Informationen in Computersystemen verwendet wird. Siehe <<uuid>>.

Verifiable Credential (VC)::
ist ein manipulationssicherer Berechtigungsnachweis, dessen Urheberschaft kryptografisch verifiziert werden kann. Siehe <<verifiable-credentials>>.

Verified by GS1::
"Verified by GS1" ist eine globale Lösung, die es Einzelhändlern und Marktplätzen ermöglicht, die Identität eines Produkts und markenbezogene Produktdaten durch Abfrage der GS1 Registry-Plattform zu überprüfen. Siehe <<verified-by-gs1>>.

X.509::
ist ein ITU-T-Standard für eine Public-Key-Infrastruktur zum Erstellen digitaler Zertifikate. Siehe <<X509>>
